...
describe('<Icon />', () => {
  it('should render an icon with default props', () => {
    ...
    expect(pathProps).toHaveProperty('d', SVGIcons[`WICON_${name}`])
  })
  ...
  it('should accept a color to paint the icon', () => {
    ...
    const color = 'blue'
    const wrapper = shallow(<Icon color={color} name={name} size={size} />)
  ...
  it('should accept extra props and execute callbacks as normal', () => {
    ...
    const wrapper = shallow(<Icon name={name} onClick={onClick} someRandomProp="HELLO" />)
